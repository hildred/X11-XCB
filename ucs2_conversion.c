 ...
 *
 * borrowed from i3 - an improved dynamic tiling window manager
 * © 2009 Michael Stapelberg and contributors (see also: LICENSE.i3)
 * © 2015 Ben Hildred
 *
 * The main reason we are modifing as oposed to just linking is for
 * better perl SV support when linking with xs. Mostly just length 
 * calculation.
 *
 */
#include <err.h>
 ...
 *
 */
xcb_char2b_t *convert_SV_to_ucs2(SV *input_sv, size_t *real_strlen) {
    /* Get the input buffer size from the SV */
    size_t input_size;
    char * input=SvPV(input_sv,input_size);

    /* Calculate the output buffer size and allocate the buffer */
    size_t buffer_size = input_size * sizeof(xcb_char2b_t);
    xcb_char2b_t *buffer = malloc(buffer_size);
    ...
    if (ucs2_conversion_descriptor == (iconv_t)-1) {
        ...
        if (ucs2_conversion_descriptor == (iconv_t)-1)
            Perl_croak(aTHX_ "Error opening the conversion context");
    } else {
        /* Reset the existing conversion descriptor */
